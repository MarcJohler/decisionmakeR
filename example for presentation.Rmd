...
# function to combine the requirements of different types of variables to generate a preference system
generate_preference_system <- function(number_of_acts, number_of_states, cardinal_measures = NULL, ordinal_measures = NULL, binary_measures = NULL, independent = FALSE) {
  # the number of acts and states must be represented by strictly positive integerish values
  assert_count(number_of_acts, positive = TRUE)
  assert_count(number_of_states, positive = TRUE)
  
  if (is.null(cardinal_measures) && is.null(ordinal_measures) && is.null(binary_measures)) {
    stop("please define at least one of 'cardinal_measures', 'ordinal_measure', 'binary_measures'")
  }
  if (!is.null(cardinal_measures)) {
    check_preference_measure(number_of_acts, number_of_states, cardinal_measures, type = "cardinal")
  }
  if (!is.null(ordinal_measures)) {
    check_preference_measure(number_of_acts, number_of_states, ordinal_measures, type = "ordinal")
  }
  if (!is.null(binary_measures)) {
    check_preference_measure(number_of_acts, number_of_states, binary_measures, type = "binary")
  }
  ...
  number_of_coordinates <- length(all_coordinates)

  cardibin_comparison <- matrix(0, nrow = number_of_coordinates, ncol = number_of_coordinates)
  colnames(cardibin_comparison) <- all_coordinates
  rownames(cardibin_comparison) <- all_coordinates
  ordibin_comparison <- matrix(0, nrow = number_of_coordinates, ncol = number_of_coordinates)
  colnames(ordibin_comparison) <- all_coordinates
  rownames(ordibin_comparison) <- all_coordinates
  binary_comparison <- matrix(0, nrow = number_of_coordinates, ncol = number_of_coordinates)
  colnames(binary_comparison) <- all_coordinates
  rownames(binary_comparison) <- all_coordinates

  cardibin_measures <- NULL
  if (!is.null(cardinal_measures) || (!is.null(binary_measures) && independent == TRUE)) {
    if (!is.null(cardinal_measures) && (!is.null(binary_measures) && independent == TRUE)) {
      cardibin_measures <- cbind(cardinal_measures, binary_measures)
    }
    else if (is.null(cardinal_measures)) {
      cardibin_measures <- binary_measures
    }
    else {
      cardibin_measures <- cardinal_measures
    }

    # find entries which have higher value in every cardinal and binary variable
    # --> if this code junk is used, binary variables won't be used in the next code junk
    cardibin_strict <- matrix(NA, 0, 4) %>% as.data.frame()
    cardibin_indifferent <- matrix(NA, 0, 4) %>% as.data.frame()
    for (i in 1:(number_of_coordinates - 1)) {
      for (j in (i + 1):(number_of_coordinates)) {
        compare_ij <- cardibin_measures[i, ] - cardibin_measures[j, ]
        if (prod(compare_ij >= 0) == 1 || prod(compare_ij <= 0) == 1) {
          # part for R1
          cardibin_comparison[i, j] <- sum(compare_ij)
          cardibin_comparison[j, i] <- (-1) * sum(compare_ij)
          # part for R2
          if (prod(compare_ij == 0) == 0 && i != number_of_coordinates - 1) {
            if (prod(compare_ij >= 0) == 1) {
              for (k in (i + 1):(number_of_coordinates - 1)) {
                for (l in (k + 1):number_of_coordinates) {
                  compare_kl <- cardibin_measures[k, ] - cardibin_measures[l, ]
                  if (prod(compare_kl == 0) == 0) {
                    if (prod(compare_kl >= 0) == 1) {
                      # check if requirement is non-trivial
                      if (i != k && j != l && (i != l || k != j)) {
                        compare_ijkl <- compare_ij - compare_kl
                        if (prod(compare_ijkl == 0) == 1) {
                          cardibin_indifferent <- rbind(cardibin_indifferent, c(i, j, k, l))
                          cardibin_indifferent <- rbind(cardibin_indifferent, c(k, l, i, j))
                        }
                        else if (prod(compare_ijkl >= 0) == 1) {
                          cardibin_strict <- rbind(cardibin_strict, c(i, j, k, l))
                        }
                        else if (prod(compare_ijkl <= 0) == 1) {
                          cardibin_strict <- rbind(cardibin_strict, c(k, l, i, j))
                        }
                      }
                    }
                    if (prod(compare_kl <= 0) == 1) {
                      ...
                      if (i != l && j != k && (i != k || j != l)) {
                        compare_ijkl <- compare_ij + compare_kl
                        if (prod(compare_ijkl == 0) == 1) {
                          cardibin_indifferent <- rbind(cardibin_indifferent, c(i, j, l, k))
                          cardibin_indifferent <- rbind(cardibin_indifferent, c(l, k, i, j))
                        }
                        else if (prod(compare_ijkl >= 0) == 1) {
                          cardibin_strict <- rbind(cardibin_strict, c(i, j, l, k))
                        }
                        else if (prod(compare_ijkl <= 0) == 1) {
                          cardibin_strict <- rbind(cardibin_strict, c(l, k, i, j))
                        }
                      }
                    }
                  }
                ...
                for (l in (k + 1):number_of_coordinates) {
                  ...
                  if (prod(compare_kl == 0) == 0) {
                    if (prod(compare_kl >= 0) == 1) {
                      # check if requirement is non-trivial
                      if (i != l && j != k && (i != k || l != j)) {
                        compare_ijkl <- (-1) * compare_ij - compare_kl
                        if (prod(compare_ijkl == 0) == 1) {
                          cardibin_indifferent <- rbind(cardibin_indifferent, c(j, i, k, l))
                          cardibin_indifferent <- rbind(cardibin_indifferent, c(k, l, j, i))
                        }
                        else if (prod(compare_ijkl >= 0) == 1) {
                          cardibin_strict <- rbind(cardibin_strict, c(j, i, k, l))
                        }
                        else if (prod(compare_ijkl <= 0) == 1) {
                          cardibin_strict <- rbind(cardibin_strict, c(k, l, j, i))
                        }
                      }
                    }
                    if (prod(compare_kl <= 0) == 1) {
                      ...
                      if (i != k && j != l && (i != l || j != k)) {
                        compare_ijkl <- (-1) * compare_ij + compare_kl
                        if (prod(compare_ijkl == 0) == 1) {
                          cardibin_indifferent <- rbind(cardibin_indifferent, c(j, i, l, k))
                          cardibin_indifferent <- rbind(cardibin_indifferent, c(l, k, j, i))
                        }
                        else if (prod(compare_ijkl >= 0) == 1) {
                          cardibin_strict <- rbind(cardibin_strict, c(j, i, l, k))
                        }
                        else if (prod(compare_ijkl <= 0) == 1) {
                          cardibin_strict <- rbind(cardibin_strict, c(l, k, j, i))
                        }
                      }
                    }
                  }
          ...
          cardibin_comparison[i, j] <- NA
          cardibin_comparison[j, i] <- NA
        }
      }
    }

    cardibin_comparison <- sign(cardibin_comparison)
    names(cardibin_strict) <- c("A", "B", "C", "D")
    names(cardibin_indifferent) <- c("A", "B", "C", "D")
  }

  ordibin_measures <- NULL
  if (!is.null(ordinal_measures) || (!is.null(binary_measures) && independent == FALSE)) {
    if (!is.null(ordinal_measures) && (!is.null(binary_measures) && independent == FALSE)) {
      ordibin_measures <- cbind(ordinal_measures, binary_measures)
    }
    else if (is.null(ordinal_measures)) {
      ordibin_measures <- binary_measures
    }
    else {
      ordibin_measures <- ordinal_measures
    }
    # find entries which have higher value in every ordinal and binary variable
    # r2 relation could be wrong for ordinal variables --> need to check
    ordibin_strict <- matrix(NA, 0, 4) %>% as.data.frame()
    ordibin_indifferent <- matrix(NA, 0, 4) %>% as.data.frame()
    for (i in 1:(number_of_coordinates - 1)) {
      for (j in (i + 1):(number_of_coordinates)) {
        compare_ij <- sign(ordibin_measures[i, ] - ordibin_measures[j, ])
        if (prod(compare_ij >= 0) == 1 || prod(compare_ij <= 0) == 1) {
          # part for R1
          ordibin_comparison[i, j] <- sum(compare_ij)
          ordibin_comparison[j, i] <- (-1) * sum(compare_ij)
        }
        else {
          ordibin_comparison[i, j] <- NA
          ordibin_comparison[j, i] <- NA
        }
      }
    }
    ordibin_comparison <- sign(ordibin_comparison)

    ordibin_strict <- matrix(NA, 0, 4) %>% as.data.frame()
    ordibin_indifferent <- matrix(NA, 0, 4) %>% as.data.frame()
    for (i in 1:number_of_coordinates) {
      for (k in 1:number_of_coordinates) {
        if (i != k && prod(ordibin_measures[i, ] - ordibin_measures[k, ] >= 0) == 1) {
          for (l in 1:number_of_coordinates) {
            if (k != l && prod(ordibin_measures[k, ] - ordibin_measures[l, ] >= 0) == 1) {
              for (j in 1:number_of_coordinates) {
                if (l != j && i != j && (i != l || j != k) && prod(ordibin_measures[l, ] - ordibin_measures[j, ] >= 0) == 1) {
                  if (prod(ordibin_measures[i, ] == ordibin_measures[k, ]) == 1 && prod(ordibin_measures[l, ] == ordibin_measures[j, ]) == 1) {
                    ordibin_indifferent <- rbind(ordibin_indifferent, c(i, j, k, l))
                  }
                  else {
                    ordibin_strict <- rbind(ordibin_strict, c(i, j, k, l))
                  }
                }
              }
            }
    ...
    # only include this if all variables are independent

    if (independent == TRUE) {
      for (i in 1:(number_of_coordinates - 2)) {
        for (j in (i + 1):(number_of_coordinates - 1)) {
          if (prod(ordibin_measures[i, ] - ordibin_measures[j, ] == 0) == 1) {
            for (k in j:(number_of_coordinates - 1)) {
              for (l in (k + 1):number_of_coordinates) {
                # insert all possible combinations
                if (prod(ordibin_measures[k, ] - ordibin_measures[l, ] == 0) == 1) {
                  ordibin_indifferent <- rbind(ordibin_indifferent, c(i, j, k, l))
                  ordibin_indifferent <- rbind(ordibin_indifferent, c(i, j, l, k))
                  ordibin_indifferent <- rbind(ordibin_indifferent, c(j, i, k, l))
                  ordibin_indifferent <- rbind(ordibin_indifferent, c(j, i, l, k))
                  ordibin_indifferent <- rbind(ordibin_indifferent, c(k, l, i, j))
                  ordibin_indifferent <- rbind(ordibin_indifferent, c(l, k, i, j))
                  ordibin_indifferent <- rbind(ordibin_indifferent, c(k, l, j, i))
                  ordibin_indifferent <- rbind(ordibin_indifferent, c(l, k, j, i))
                }
              }
            }
          }
    ...
    names(ordibin_strict) <- c("A", "B", "C", "D")
    names(ordibin_indifferent) <- c("A", "B", "C", "D")
  }

  # define R1 relation
  r1_strict <- list()
  r1_indifferent <- list()
  if (is.null(ordinal_measures) && (is.null(binary_measures) || independent == TRUE)) {
    for (i in 1:(number_of_coordinates - 1)) {
      for (j in (i + 1):number_of_coordinates) {
        if (!is.na(cardibin_comparison[i, j])) {
          if (cardibin_comparison[i, j] == 0) {
            r1_indifferent[[length(r1_indifferent) + 1]] <- c(all_coordinates[i], all_coordinates[j])
          }
          else if (cardibin_comparison[i, j] >= 0) {
            r1_strict[[length(r1_strict) + 1]] <- c(all_coordinates[i], all_coordinates[j])
          }
          else if (cardibin_comparison[j, i] >= 0) {
            r1_strict[[length(r1_strict) + 1]] <- c(all_coordinates[j], all_coordinates[i])
          }
        }
      }
    }
  }
  else if (is.null(cardinal_measures) && (is.null(binary_measures) || independent == FALSE)) {
    for (i in 1:(number_of_coordinates - 1)) {
      for (j in (i + 1):number_of_coordinates) {
        if (!is.na(ordibin_comparison[i, j])) {
          if (ordibin_comparison[i, j] == 0) {
            r1_indifferent[[length(r1_indifferent) + 1]] <- c(all_coordinates[i], all_coordinates[j])
          }
          else if (ordibin_comparison[i, j] >= 0) {
            r1_strict[[length(r1_strict) + 1]] <- c(all_coordinates[i], all_coordinates[j])
          }
          else if (ordibin_comparison[j, i] >= 0) {
            r1_strict[[length(r1_strict) + 1]] <- c(all_coordinates[j], all_coordinates[i])
          }
        }
      }
    ...
    for (i in 1:(number_of_coordinates - 1)) {
      for (j in (i + 1):number_of_coordinates) {
        if (!is.na(cardibin_comparison[i, j]) && !is.na(ordibin_comparison[i, j])) {
          if (cardibin_comparison[i, j] == 0 && ordibin_comparison[i, j] == 0) {
            r1_indifferent[[length(r1_indifferent) + 1]] <- c(all_coordinates[i], all_coordinates[j])
          }
          else if (cardibin_comparison[i, j] >= 0 && ordibin_comparison[i, j] >= 0) {
            r1_strict[[length(r1_strict) + 1]] <- c(all_coordinates[i], all_coordinates[j])
          }
          else if (cardibin_comparison[j, i] >= 0 && ordibin_comparison[j, i] >= 0) {
            r1_strict[[length(r1_strict) + 1]] <- c(all_coordinates[j], all_coordinates[i])
          }
        }
      }
  ...
  if (length(r1_strict) == 0) {
    r1_strict <- NULL
  }
  if (length(r1_indifferent) == 0) {
    r1_indifferent <- NULL
  }

  if (!is.null(cardibin_measures) && !is.null(ordibin_measures)) {
    cardibin_all <- union(cardibin_strict, cardibin_indifferent)
    ordibin_all <- union(ordibin_strict, ordibin_indifferent)
    r2_strict_matrix <- union(intersect(cardibin_all, ordibin_strict), intersect(cardibin_strict, ordibin_all)) %>% as.matrix()
    r2_indifferent_matrix <- intersect(cardibin_indifferent, ordibin_indifferent) %>% as.matrix()
  }
  else if (!is.null(cardibin_measures)) {
    r2_strict_matrix <- cardibin_strict %>% as.matrix()
    r2_indifferent_matrix <- cardibin_indifferent %>% as.matrix()
  }
  else if (!is.null(ordibin_measures)) {
    r2_strict_matrix <- ordibin_strict %>% as.matrix()
    r2_indifferent_matrix <- ordibin_indifferent %>% as.matrix()
  }

  r2_indifferent <- list()
  if (nrow(r2_indifferent_matrix) > 0) {
    for (i in 1:nrow(r2_indifferent_matrix)) {
      for (j in 1:4) {
        r2_indifferent_matrix[i, j] <- all_coordinates[r2_indifferent_matrix[i, j]]
      }
      r2_indifferent[[length(r2_indifferent) + 1]] <- r2_indifferent_matrix[i, ]
    }
  }
  else {
    r2_indifferent <- NULL
  ...
  r2_strict <- list()
  if (nrow(r2_strict_matrix) > 0) {
    for (i in 1:nrow(r2_strict_matrix)) {
      for (j in 1:4) {
        r2_strict_matrix[i, j] <- all_coordinates[r2_strict_matrix[i, j]]
      }
      r2_strict[[length(r2_strict) + 1]] <- r2_strict_matrix[i, ]
    }
  }
  else {
    r2_strict <- NULL
  }
  return(list(r1_strict, r1_indifferent, r2_strict, r2_indifferent))
}
...
# Example 1 to show difference when assuming independency
cardinal_preferences <- data.frame(row.names = 1:6)
cardinal_preferences$salary <- c(1000, 2000, 1500, 3500, 1500, 1800)
ordinal_preferences <- data.frame(row.names = 1:6)
ordinal_preferences$level <- c(1, 1, 2, 2, 0, 2)

pref_system1 <- generate_preference_system(2, 3, cardinal_preferences, ordinal_preferences)
check_preference_system(2, 3, pref_system1[[1]], pref_system1[[2]], pref_system1[[3]], pref_system1[[4]])

pref_system2 <- generate_preference_system(2, 3, cardinal_preferences, ordinal_preferences, independent = TRUE)
check_preference_system(2, 3, pref_system2[[1]], pref_system2[[2]], pref_system2[[3]], pref_system2[[4]])


# Example 2 to show difference when assuming independency
binary_preferences <- data.frame(row.names = 1:6)
binary_preferences$b1 <- c(TRUE, FALSE, FALSE, TRUE, FALSE, TRUE)
binary_preferences$b2 <- c(TRUE, TRUE, FALSE, FALSE, FALSE, TRUE)
binary_preferences$b3 <- c(FALSE, FALSE, TRUE, TRUE, FALSE, TRUE)
ordinal_preferences <- data.frame(row.names = 1:6)
ordinal_preferences$level <- c(1, 1, 2, 2, 0, 2)

pref_system1 <- generate_preference_system(2, 3, ordinal_measures = ordinal_preferences, binary_measures = binary_preferences)
check_preference_system(2, 3, pref_system1[[1]], pref_system1[[2]], pref_system1[[3]], pref_system1[[4]])

pref_system2 <- generate_preference_system(2, 3, ordinal_measures = ordinal_preferences, binary_measures = binary_preferences, independent = TRUE)
check_preference_system(2, 3, pref_system2[[1]], pref_system2[[2]], pref_system2[[3]], pref_system2[[4]])
...
ps_local_admissibility <- function(number_of_acts, number_of_states, r1_strict, r1_indifferent = NULL, r2_strict = NULL, r2_indifferent = NULL, mode = c("utility","loss"), relation = c("p1u1", "u1p1", "u1pn", "p1un", "unp1", "pnu1", "unpn", "pnun"), a1 = NULL, b1 = NULL, a2 = NULL, b2 = NULL, ordinal_structure = NULL, boundaries = matrix(c(0, 1), nrow = number_of_states, ncol = 2, byrow = TRUE)) {
  ...
  #check if relation is one of "p1u1", "u1p1", "u1pn", "p1un", "unp1", "pnu1", "unpn", "pnun"
  ...
  if (relation %in% c("pnun", "unpn")) {
    ...
    to_consider <- 1:number_of_acts

    i <- 1
    while (i <= length(to_consider)) {
      i2 <- 1
      while (i2 <= length(to_consider)) {
        if (i != i2) {
          indx_i <- to_consider[i]
          indx_i2 <- to_consider[i2]
          remember <- FALSE
          for (j in 1:nrow(extreme_points)) {
            objective <- rep(0, number_of_var)
            objective[((indx_i - 1) * number_of_states + 1):(indx_i * number_of_states)] <- extreme_points[j, ]
            objective[((indx_i2 - 1) * number_of_states + 1):(indx_i2 * number_of_states)] <- (-1) * extreme_points[j, ]
          ...
          if (remember == TRUE) {
            to_consider <- setdiff(to_consider, indx_i2)
            if (i > i2) {
              i <- i - 1
            }
            i2 <- i2 - 1
  ...
  else if (relation %in% c("u1pn")) {
    extreme_points <- extreme_point_finder_rcdd(number_of_states, a1, b1, a2, b2, boundaries, ordinal_structure)
    constr_vec <- c(constr_vec, rep(0, nrow(extreme_points)))
    objective <- c(rep(0, number_of_var - 1), 1)

    not_admissible <- c()

    if (mode == "utility") {
      constr_dir <- c(constr_dir, rep(">=", nrow(extreme_points)))
    }
    else if (mode == "loss") {
      constr_dir <- c(constr_dir, rep("<=", nrow(extreme_points)))
    }
    ...
    for (i in 1:number_of_acts) {
      for (i2 in 1:number_of_acts) {
        if (i != i2) {
          ...
          optimization <- lp("max", objective, constr_mat_iteration, constr_dir, constr_vec)
          if (optimization$objval > 0) {
            not_admissible <- c(not_admissible, i2)
          }
        }
      }
    }
      ...
      for (i2 in 1:number_of_acts) {
        if (i != i2) {
          ...
          for (j in 1:nrow(extreme_points)) {
            objective <- rep(0, number_of_var)
            objective[((i - 1) * number_of_states + 1):(i * number_of_states)] <- extreme_points[j, ]
            objective[((i2 - 1) * number_of_states + 1):(i2 * number_of_states)] <- (-1) * extreme_points[j, ]
      ...
      for (i2 in 1:number_of_acts) {
        if (i != i2) {
          for (j in 1:nrow(extreme_points)) {
            ...
            optimization <- lp("min", objective, constr_mat, constr_dir, constr_vec)
            if (optimization$objval >= 0) {
              if (lp("max", objective, constr_mat, constr_dir, constr_vec)$objval > 0) {
                not_admissible <- c(not_admissible, i2)
                break
              }
            }
  ...
  else if (relation %in% c("unp1")) { # big question how to solve it#
    return("not supported currently")
  }
  else if (relation %in% c("p1u1", "u1p1")) {
    extreme_points <- extreme_point_finder_rcdd(number_of_states, a1, b1, a2, b2, boundaries, ordinal_structure)
      ...
      for (i2 in 1:number_of_acts) {
        ...
        if (i != i2) {
          for (j in 1:nrow(extreme_points)) {
            ...
            if (optimization$objval > max_optimization) {
              max_optimization <- optimization$objval
            }
          }
          if (max_optimization > 0) {
            not_admissible <- c(not_admissible, i2)
          }
        }
      }
...
# define the input for the probabilistic information
```{r second}
